...
import baritonex.utils.XCartesian;
import baritonex.utils.XMapPopulator;
...
public class BlockState
{
    ...
    public BlockState(Block blockIn, IProperty... properties)
    {
        ...
        for (List<Comparable> list1 : XCartesian.cartesianProduct(this.getAllowedValues()))
        {
            Map<IProperty, Comparable> map1 = XMapPopulator.<IProperty, Comparable>createMap(this.properties, list1);
    ...
    {
        return Objects.toStringHelper(this).add("block", Block.blockRegistry.getNameForObject(this.block)).add("properties", "nah").toString();
    }
    ...
    static class StateImplementation extends BlockStateBase
    {
        ...
        private StateImplementation(Block blockIn, ImmutableMap<IProperty, Comparable> propertiesIn)
        {
            ...
            this.hashCode = propertiesIn.hashCode();
        }
        ...
        public <T extends Comparable<T>> T getValue(IProperty<T> property)
        {
            ...
            {
                throw new IllegalArgumentException("Cannot get property " + property + " as it does not exist in " + XBlockStateSerializer.getBlockState(this.block));
            }
            else
            {
        ...
        public <T extends Comparable<T>, V extends T> IBlockState withProperty(IProperty<T> property, V value)
        {
            if (!this.properties.containsKey(property))
            {
                throw new IllegalArgumentException("Cannot set property " + property + " as it does not exist in " + XBlockStateSerializer.getBlockState(this.block));
            }
