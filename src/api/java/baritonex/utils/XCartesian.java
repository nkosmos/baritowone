...
public class XCartesian
{
    ...
    {
        return Iterables.transform(arrays, new XCartesian.GetList());
    }
    ...
    static class Product<T> implements Iterable<T[]>
        {
            ...
            {
                return (Iterator<T[]>)(this.iterables.length <= 0 ? Collections.singletonList((Object[])XCartesian.createArray(this.clazz, 0)).iterator() : new XCartesian.Product.ProductIterator(this.clazz, this.iterables));
            }
            ...
            static class ProductIterator<T> extends UnmodifiableIterator<T[]>
                {
                    ...
                    private ProductIterator(Class<T> clazz, Iterable <? extends T > [] iterables)
                    {
                        this.index = -2;
                        this.iterables = iterables;
                        this.iterators = (Iterator[])XCartesian.createArray(Iterator.class, this.iterables.length);
                        ...
                        this.results = XCartesian.createArray(clazz, this.iterators.length);
                    }
